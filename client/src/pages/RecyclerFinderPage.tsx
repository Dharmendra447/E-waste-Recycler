...
export function RecyclerFinderPage() {
  ...
  const fetchRecyclers = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/recyclers?category=${encodeURIComponent(category)}`, { credentials: 'include' });
    ...
    <div className="container mx-auto space-y-8 px-4 py-8">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        <section className="space-y-4">
          <div className="flex gap-2"><Input value={category} onChange={(event) => setCategory(event.target.value)} placeholder="Search by category, e.g. batteries" /><Button variant="outline" onClick={fetchRecyclers}>Search</Button></div>
          {isLoading ? <p className="text-muted-foreground">Finding nearby recyclers...</p> : recyclers.length === 0 ? <Card><CardContent className="pt-6 text-muted-foreground">No active recyclers match this category.</CardContent></Card> : recyclers.map((recycler) => <Card key={recycler.id}><CardHeader><div className="flex items-start justify-between gap-4"><div><CardTitle>{recycler.name}</CardTitle><CardDescription className="mt-1"><MapPin className="mr-1 inline h-4 w-4" />{recycler.address || recycler.city || 'Location not provided'}</CardDescription></div><Badge variant="default">Active</Badge></div></CardHeader><CardContent className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="text-sm text-muted-foreground"><p><Recycle className="mr-1 inline h-4 w-4" />Accepts: {recycler.accepted_categories || 'Electronic waste'}</p><p><Phone className="mr-1 inline h-4 w-4" />{recycler.email}</p></div><Button onClick={() => setSelectedRecycler(recycler)}>Schedule pickup</Button></CardContent></Card>)}
        </section>
